...
class Food {
    ...
    printFood() {
        c.fillStyle = '#fff'
        c.fillRect(this.location[0], this.location[1], Game.cellSize(), Game.cellSize())
        console.log('food printed')
    }
}
...
class Game {
    ...
    cycle() {
        var foodCheck = false
        if (self.food) {foodCheck = isEqualArray2(self.snake.head, self.food.location)}
        ...
        if (self.snake.moveSnake(!foodCheck)) {
            if (self.snake.tail_deleted) c.clearRect(self.snake.tail_deleted[0], self.snake.tail_deleted[1], 10, 10)
